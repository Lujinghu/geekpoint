...
def create_shop(request):
    ...
    if form.is_valid():
        ...
        shop.shop_managers.add(request.user) #这里出错了，因为多对多字段是一个set，所以不能直接这样赋值
...
def charge_food(request, shop_id):
    ...
    food_category_list = shop.foodcategory_set()
    return render(request, 'geekpoint/charge_food.html', {
        'shop': shop, 'food_list': food_list, 'food_category_list': food_category_list
    })
...
def create_food(request, shop_id):
    ...
    if form.is_valid():
        ...
        food_category = get_object_or_404(models.FoodCategory, id=request.POST.get('food_category_id'))
        food.shop = shop
...
def edit_food(request, food_id):
    ...
    check_manager(request, food.shop)
    if request.method == 'GET':
        #渲染一张食物视图表单
...
def shop_mark_order(request, order_id):
    ...
    check_manager(request, order.shop)
...
def order_food(request, shop_id):
    ...
    if request.method == 'GET':
        ...
        if food_category_id:
            ...
            food_list = models.Food.objects.query_by_shop(shop)
        food_category_list = shop.foodcategory_set.all()
        return render(request, 'geekpoint/order_food.html', {
            'food_list': food_list,
            'shop': shop,
            'food_category_list': food_category_list
        })
